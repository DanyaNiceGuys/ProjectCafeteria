...
auto result = calc.calculate(1, {}, 1);
ASSERT_TRUE(result.has_value());
EXPECT_DOUBLE_EQ(*result, 220.0);
}

TEST(PriceCalculatorTest, PriceWithModifier) {
PriceCalculator calc(makeTestVariants(), makeTestModifiers());
auto result = calc.calculate(1, {1}, 1);
ASSERT_TRUE(result.has_value());
EXPECT_DOUBLE_EQ(*result, 300.0);
}

TEST(PriceCalculatorTest, PriceScalesWithQuantity) {
PriceCalculator calc(makeTestVariants(), makeTestModifiers());
auto result = calc.calculate(1, {}, 3);
ASSERT_TRUE(result.has_value());
EXPECT_DOUBLE_EQ(*result, 660.0);
}

TEST(PriceCalculatorTest, UnknownVariantReturnsNullopt) {
PriceCalculator calc(makeTestVariants(), makeTestModifiers());
auto result = calc.calculate(999, {}, 1);
EXPECT_FALSE(result.has_value());   // nullopt, а не 0.0
}

TEST(OrderBuilderTest, BuildsValidOrder) {
PriceCalculator calc(makeTestVariants(), makeTestModifiers());
double price = calc.calculate(1, {1}, 1).value();  // .value() извлекает из optional
